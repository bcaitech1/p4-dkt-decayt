...
class Preprocess:
    ...
    def __feature_engineering(self, *dfs, is_train, fversion):
        if is_train:
            df = dfs[0]
        if fversion == 1:
    ...
    def __feature_tag_mean(self, *dfs, is_train):
        if is_train:
            df = dfs[0]
    ...
    def load_data_from_file(self, *file_names, is_train=True):
        if is_train:
            file_name = file_names[0][0]
            csv_file_path = os.path.join(self.args.data_dir, file_name)
            df = pd.read_csv(csv_file_path)     #, nrows=100000)
            df = self.__feature_engineering(df, is_train=is_train, fversion=self.args.fversion)
        else:
            test_file_name, train_file_name = file_names[0]
            ...
            test_df = pd.read_csv(test_csv_file_path)     #, nrows=100000)
            train_df = pd.read_csv(train_csv_file_path)     #, nrows=100000)
            df = self.__feature_engineering(test_df, train_df, is_train=is_train, fversion=self.args.fversion)
        ...
        self.args.n_cont = 1  # 수치형 컬럼 갯수를 지정해야 하는데 우선 하드코딩

        df = df.sort_values(by=['userID', 'Timestamp'], axis=0)
        columns = ['userID', 'assessmentItemID', 'testId', 'answerCode', 'KnowledgeTag' ,'tag_mean']
...
class DKTDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        test, question, tag, correct, tag_mean = row[0], row[1], row[2], row[3], row[4]
        # test, question, tag, correct = row[0], row[1], row[2], row[3]

        # cate_cols = [test, question, tag, correct]
        cate_cols = [test, question, tag, correct, tag_mean]  # tag_mean은 수치형이지만 임시적으로 추가
